...
window.TowerDefense = (function($){
    ...
    td.Start = function(){
		...
		for(var i=0;i<bad_guys.length;i++){
			//setup bad_guy hit points
            bad_guys[i].current_hp = bad_guys[i].hp;
        }
    ...
    td.ShowBadGuysOnMap = function(){
        ...
        for(var i=0;i<bad_guys.length;i++){
            ...
            if(bad_guys[i].current_hp>0){
                ...
                //add at new pos
				log(bad_guys[i].current_hp + ' - ' + bad_guys[i].hp);
				var bad_guy_life = (bad_guys[i].current_hp / bad_guys[i].hp)*50;
				log(bad_guy_life);
                $('#'+map_path[bad_guys[i].current_position]).append("<div><div class='lifebar'><div class='lifebar-meter' style='width:"+bad_guy_life+"px'></div></div><img src='images/"+bad_guys[i].image+"' class='"+bad_guys[i].style+"'></img></div>");
            ...
            if((bad_guys[i].current_position >= map_path.length) && (bad_guys[i].current_hp > 0) && bad_guys[i].madeit === 0){
                bad_guys[i].madeit = 1;
                td.missedBadGuys++;
        ...
        for(var i=0;i<bad_guys.length;i++){
            for(var w=0;w<weapons.length;w++){
                if(td.IsHit(bad_guys[i].currentCell,weapons[w].position,weapons[w].range)){
                    weapons[w].hits++;
                    bad_guys[i].current_hp = bad_guys[i].current_hp - weapons[w].damage; //take away hit points
    ...
    td.IsWaveOver = function(){
        ...
        if(bad_guys.length>0){
            ...
            for(var i=0;i<bad_guys.length;i++){
                if(bad_guys[i].current_hp>0 && bad_guys[i].current_position < map_path.length){
                    result = false;
                }
            }       
